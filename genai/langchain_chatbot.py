...
# ── SQL Agent ────────────────────────────────────────────────
def get_sql_agent(admin_id: int, business_name: str, rag_context: str = ""):
    llm = get_llm()
    db  = get_sql_database(admin_id)
    ...
    if rag_context:
        rag_block = f"""
RETRIEVED BUSINESS KNOWLEDGE (use this to enrich your answer):
{rag_context}
"""
...
# ── Keyword-based routing (replaces BERT intent classifier) ──
DATA_KEYWORDS = [
    "revenue", "sales", "orders", "product", "customer",
    "how much", "how many", "top", "best", "worst",
    "total", "average", "month", "year", "show", "list",
...
def langchain_chat(
        ...
        chat_history : list,
        use_sql_agent: bool = True,
        db_engine            = None,    # SQLAlchemy engine, passed to RAG
) -> dict:
    """
    ...
    """
    needs_sql = question_needs_sql(question) and use_sql_agent

    # ── RAG retrieval ─────────────────────────────────────────
    rag_context = ""
    try:
        rag = get_rag_engine(admin_id, db_engine)
        rag_context = rag.retrieve(question, top_k=4)
    except Exception:
        rag_context = ""   # degrade gracefully

    # ── LLM call ──────────────────────────────────────────────
    answer = ""
    try:
        if needs_sql:
            agent = get_sql_agent(admin_id, business_name, rag_context)
            full_question = (
                f"{question}\n\n"
                f"Rules:\n"
                f"- Filter every SQL query by admin_id={admin_id}\n"
                f"- Use correct date functions for time-based queries\n"
                f"- Format output as clean readable text\n"
                f"- Never mention admin_id in the response, "
                f'use business name "{business_name}"\n'
                f"- Give a business insight after the answer"
            )
            lc_history = []
            for prev in chat_history[-4:]:
                lc_history.append(HumanMessage(content=prev["question"]))
            ...
            for pat in [
                f"admin_id = {admin_id}", f"admin_id={admin_id}",
                f"(admin_id = {admin_id})", f"(admin_id={admin_id})",
                f"for admin_id {admin_id}",
            ]:
                answer = answer.replace(pat, business_name)
            answer = answer.strip()
            ...
            prompt = (
                f"You are an intelligent business analytics advisor for "
                f"{business_name}.\n"
                f"You help the store owner understand their sales data "
                f"and improve their business.\n"
                f"{rag_block}\n"
                f"Previous conversation:\n{history_str}\n\n"
                f"Current question: {question}\n\n"
                f"Give a helpful, specific, and actionable answer.\n"
                f"Use the retrieved knowledge above to add context.\n"
                f"Be conversational but professional.\n"
                f"Never mention admin_id in your response."
            )
            response = llm.invoke(prompt)
        ...
        elif "iteration limit" in err.lower() or "time limit" in err.lower():
            answer = (
                "⚠️ Query timed out. "
                "Please try a simpler question like 'show top 5 products'."
            )
        else:
            answer = (
                f"I encountered an error: {err}\n\n"
                "Please try rephrasing your question."
            )
