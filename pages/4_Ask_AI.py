...
# ============================================================
# pages/4_Ask_AI.py  (RAG-enhanced — BERT removed)
...
from genai.langchain_chatbot import langchain_chat
from genai.rag_engine        import get_rag_engine
from db.connection           import engine as db_engine   # SQLAlchemy engine
...
.empty-icon  { font-size:52px;margin-bottom:16px; }
.empty-title { font-size:17px;font-weight:500;color:#374151;margin-bottom:8px; }
...
with st.sidebar:
    ...
    <div style="padding:20px 4px 16px">
        <div style="display:flex;align-items:center;gap:10px;margin-bottom:14px">
            ...
            <div>
                <div style="font-size:13px;font-weight:500;color:#e5e5e5">{business_name}</div>
                <div style="font-size:11px;color:#4b5563;margin-top:1px">@{username}</div>
            </div>
        </div>
        <div style="height:1px;background:#1a1a1a;margin-bottom:14px"></div>
    </div>
    ...
    st.markdown("""<div style="font-size:10px;color:#374151;text-transform:uppercase;
                letter-spacing:0.8px;font-weight:500;padding:0 4px;
                margin-bottom:6px">Menu</div>""", unsafe_allow_html=True)
    st.page_link("pages/2_Upload_Data.py",    label="📂  Upload Data")
    st.page_link("pages/3_Dashboard.py",      label="📊  Dashboard")
    st.page_link("pages/4_Ask_AI.py",         label="🤖  Ask AI")
    st.page_link("pages/5_Upload_History.py", label="📋  Upload History")

    st.markdown("""<div style="height:1px;background:#1a1a1a;margin:14px 0"></div>
    <div style="font-size:10px;color:#374151;text-transform:uppercase;
                letter-spacing:0.8px;font-weight:500;padding:0 4px;
                margin-bottom:8px">Settings</div>""", unsafe_allow_html=True)

    use_sql = st.toggle(
        "Use SQL Agent", value=True,
        help="ON = queries DB automatically. OFF = general advice only"
    )
    show_rag = st.toggle(
        "Show RAG context", value=False,
        help="Show retrieved knowledge chunks used for each answer"
    )
    ...
    st.markdown("""<div style="font-size:10px;color:#374151;text-transform:uppercase;
                letter-spacing:0.8px;font-weight:500;padding:0 4px;
                margin-bottom:8px">RAG Index</div>""", unsafe_allow_html=True)

    if st.button("🔄 Rebuild RAG index", use_container_width=True):
        with st.spinner("Rebuilding knowledge index..."):
            rag = get_rag_engine(admin_id, db_engine, force_rebuild=True)
            n   = len(rag.chunks)
        st.success(f"Index rebuilt — {n} chunks")

    rag_status = st.session_state.get(f"rag_built_{admin_id}", False)
...
st.markdown("""
<p class="page-title">🤖 AI Business Advisor</p>
<p class="page-sub">Powered by LangChain + Groq/LLaMA · FAISS RAG</p>
""", unsafe_allow_html=True)

st.markdown("""
<div class="ai-stack-badge">
    🔗 LangChain SQL Agent &nbsp;·&nbsp;
    📚 FAISS RAG (sentence-transformers) &nbsp;·&nbsp;
    ⚡ LLaMA 3.3 70B via Groq
</div>
""", unsafe_allow_html=True)


# ── Auto-build RAG index on first load ───────────────────────
if not st.session_state.get(f"rag_built_{admin_id}"):
    with st.spinner("Building RAG knowledge index..."):
        try:
            rag = get_rag_engine(admin_id, db_engine)
            st.session_state[f"rag_built_{admin_id}"] = True
            st.session_state[f"rag_chunks_{admin_id}"] = len(rag.chunks)
        except Exception:
            st.session_state[f"rag_built_{admin_id}"] = True

n_chunks = st.session_state.get(f"rag_chunks_{admin_id}", "—")
st.caption(f"📚 RAG index: {n_chunks} knowledge chunks loaded")
...
st.markdown("""<div style="font-size:11px;font-weight:500;color:#9ca3af;
            text-transform:uppercase;letter-spacing:0.8px;
            margin-bottom:10px">Try asking</div>""", unsafe_allow_html=True)
...
if not st.session_state["chat_history"]:
    ...
    <div class="empty-wrap">
        <div class="empty-icon">🤖</div>
        <div class="empty-title">AI Business Advisor</div>
        <div class="empty-sub">
            RAG retrieves relevant context · LangChain SQL Agent queries<br>
            your database · LLaMA 3.3 70B answers<br><br>
            Ask about revenue, products, customers, trends,<br>
            or anything about your business.
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    for chat in st.session_state["chat_history"]:
        ...
        with st.chat_message("assistant"):
            ...
            if show_rag and chat.get("rag_context"):
                with st.expander("📚 RAG context used", expanded=False):
                    st.markdown(
                        f'<div class="rag-box">{chat["rag_context"]}</div>',
                        unsafe_allow_html=True
                    )
...
if active:
    ...
    with st.chat_message("assistant"):
        with st.spinner("Retrieving context · Querying..."):
            result = langchain_chat(
                question      = active,
                admin_id      = admin_id,
                business_name = business_name,
                chat_history  = st.session_state["chat_history"],
        ...
        if show_rag and result.get("rag_context"):
            with st.expander("📚 RAG context used", expanded=False):
                st.markdown(
                    f'<div class="rag-box">{result["rag_context"]}</div>',
                    unsafe_allow_html=True
                )
